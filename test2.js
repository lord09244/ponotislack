...
for(tt = 0;tt<acc.length;tt++){
  ...
  funcc[tt] = function(inst,accc,lnw01) {
  var locati = lnw01[round]
  inst.SetLocation(locati.location, function(error, coordinates) {
    inst.Heartbeat(function(err,hb) {
        var current_time_object = new Date();
        var time = current_time_object.getHours() + ":" + ("0" + current_time_object.getMinutes()).slice(-2);
        console.log("*** NEW RUN @ " + time + ":" + ("0" + current_time_object.getSeconds()).slice(-2) + " ***" + " " + accc.username + " " + locati.location.coords.latitude + " " + locati.location.coords.longitude );
        ...
        else if (err)
        {
            ...
            instance.init(accc.username, accc.password, locati.location, accc.provider, function(err) {
                if (err)
                {
...
round = (round + 1) % lnw02.length
  setInterval(funcc[tt], HEARTBEAT_TIME_INTERVAL + (tt*500),instance,acccc,lnw02);
}
